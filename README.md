...
**현재 구현**: 업로드된 파일은 **디스크에 저장하지 않고 메모리에서만 처리**합니다.

- 파일은 `BytesIO`를 통해 메모리에서 직접 읽혀 텍스트로 추출됩니다
- 처리 완료 후 파일 데이터는 메모리에서 자동으로 해제됩니다
- Streamlit 앱에서는 세션 상태에 bytes로만 임시 저장되며, 응답 후 자동 삭제됩니다

### Git 제외 설정

업로드된 파일이 실수로 Git에 커밋되지 않도록 `.gitignore`에 다음 규칙이 포함되어 있습니다:

```
# Uploaded files (임시 저장소)
uploads/
temp/
tmp/
*.pdf
...
**참고**: 만약 향후 임시 저장소를 사용하게 되더라도, 업로드된 파일은 Git에 포함되지 않습니다.
